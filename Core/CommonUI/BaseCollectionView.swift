...
public class BaseCollectionView: UICollectionView {
    ...
    public func setupAutoFooter() {
        let newFooter = MJRefreshAutoFooter()
        if isCanLoadMore {
            // load more
            newFooter.setRefreshingTarget(self, refreshingAction: #selector(mjFooterRefresh))
            newFooter.resetNoMoreData()
            self.mj_footer = newFooter
        } else {
            self.mj_footer = nil
        }
    }
